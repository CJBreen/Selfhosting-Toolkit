...
sleep 1
echo Immich will be installed by default in your home directory.
...
if [[ ${#location} == 0 ]]; then
  echo Installing to SelfService/Immich
  rootdir="/home/cbreen/SelfService"
  sleep .5
  if [[ -d "$rootdir" ]]; then
    echo SelfService folder exists. Creating Immich Folder.
    mkdir ~/SelfService/Immich
    sleep .5
    echo Done.
  else
    mkdir ~/SelfService
    mkdir ~/SelfService/Immich
    echo Created "SelfService/Immich"
  fi

  # if they specify a location for the installation path
else
  echo Installing to $location/Immich
  if [[ -d "$location" ]]; then
    mkdir ~/$location/Immich
  else
...
echo Downloading Immich docker files...
wget -O docker-compose.yml https://github.com/immich-app/immich/releases/latest/download/docker-compose.yml -o ~/SelfService/Immich/compose.yaml
sleep 1.5
echo Done.

echo Downloading Immich environment files...
wget -O .env https://github.com/immich-app/immich/releases/latest/download/example.env ~/SelfService/Immich/.env

# running the docker file itself
printf "Would you like to start up Immich? [y/n] "
read answer
